    ...
    if ($uploadOk == 1) {
        if (move_uploaded_file($_FILES['Teamlogo']['tmp_name'], $teamLogo) && move_uploaded_file($_FILES['Captainphoto']['tmp_name'], $captainPhoto)) {
            ...
            try {
                ...
                $mail->SMTPAuth = true;
				$mail->Username = 'rajporapremiert20cup@gmail.com'; // Your Gmail address
				$mail->Password = 'mpbzzvsixtjjbsac'; // Your Gmail app password
                $mail->SMTPSecure = 'tls';
                $mail->Port = 587;

                // Recipients
                $mail->setFrom('no-reply@yourdomain.com', 'Registration');
                $mail->addAddress('rajporapremiert20cup@gmail.com'); // Add a recipient
